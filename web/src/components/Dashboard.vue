          ...
          <div class="card" style="margin-top: 2rem;">
            <div class="deck-header">
              ...
              <div class="deck-controls">
                <select v-model="selectedProfile" @change="loadProfile" class="profile-select" data-cy="profile-selector">
            ...
            <div class="current-page-indicator" data-cy="current-page">
              Página actual: {{ currentPage?.name || 'Ninguna' }}
            </div>
            
            <!-- Page Navigation -->
            <div class="page-navigation" v-if="pages.length > 0" data-cy="page-nav">
              <button 
                v-for="page in pages" 
                :key="page.id"
                @click="selectPage(page)"
                :class="['page-nav-item', { active: currentPage?.id === page.id }]"
              >
                {{ page.name }}
              </button>
            </div>
            <div class="quick-actions">
              <button @click="addNewPage" class="btn btn-secondary" data-cy="add-page">
                ➕ Nueva Página
              </button>
        ...
        <div v-if="activeTab === 'settings'" class="tab-content">
          ...
          <div class="grid" style="margin-top: 2rem;">
            ...
            <div class="card">
              ...
              <div class="setting-item">
                <label>Idioma:</label>
                <select v-model="currentLanguage" @change="changeLanguage" class="input" data-cy="language-selector">
...
const loadProfile = async () => {
  ...
  try {
    ...
    const profilePages = await apiRequest(`/pages?profileId=${profile.id}`)
    pages.value = profilePages
    
    if (profilePages.length > 0) {
      currentPage.value = profilePages[0]
      const buttons = await apiRequest(`/buttons?pageId=${profilePages[0].id}`)
      currentButtons.value = buttons
...
const changeLanguage = (event: any) => {
  ...
  // Here you would typically save to settings and reload the UI with new language
